...
* Bumped `flutter_file_manager_platform_interface` to `2.1.1`
* Migrates to built-in Kotlin (removes `kotlin-android` Gradle plugin and `kotlin-gradle-plugin` buildscript classpath); uses `kotlin { compilerOptions { jvmTarget } }` block.
* Adds support for Android Gradle Plugin 9.x.
* Converts Android Gradle files to Kotlin DSL (`.gradle.kts`).
* Updates Gradle to 9.1.0, AGP to 9.0.1, Kotlin to 2.3.20, `compileSdk` to 36, `jvmTarget` to 17.
